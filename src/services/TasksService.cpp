...
TasksService::TasksService(QObject* parent) : QObject(parent), m_pNotebookService(new NotebookService(this)) {
    ...
    QFile dbFile(TasksService::DB_PATH);
    bool newDb = !dbFile.exists();

    m_database = QSqlDatabase::addDatabase("QSQLITE");
    m_database.setDatabaseName(TasksService::DB_PATH);
    m_database.open();
...
QVariantList TasksService::findAll() const {
    QVariantList tasks = m_pSda->execute("SELECT * FROM tasks ORDER BY parent_id").toList();
